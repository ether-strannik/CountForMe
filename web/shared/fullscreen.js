...
//
// Served at /shared/fullscreen.js for every board. A page opts in with one
// line and gets the whole behaviour:
//
//     <script src="/shared/fullscreen.js" defer></script>
//
// Why it is worth having: the browser's address bar and Android's system
// bars eat about a fifth of a phone screen, and a board is read at arm's
// length. Full screen needs a user gesture, which is exactly what a tap is,
// so it cannot be done for the user on load.
//
// Borrowed from the scenario board, which had it first, with two guards
// added because these boards have buttons and long lists to scroll:
//
//   - a tap on anything you can operate is not a page tap, so a fast
//     double-press on SWEEP or a project row cannot flip the screen
//   - a tap that moved is a scroll, not a tap
//
// `touch-action: manipulation` is set here rather than in each page's CSS:
...
document.addEventListener(
  ...
  (event) => {
    ...
    const moved =
      from && touch
        ? Math.hypot(touch.clientX - from.x, touch.clientY - from.y) > MOVE_PX
        : false;
    if (onControl || moved) {
      last = 0;
      return;
